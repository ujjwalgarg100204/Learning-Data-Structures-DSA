...
#include "../node/string/node_singly_s.h"
 ...
 */
Node *insertAfter(Node *node, const char *data);
...
bool contains(const LinkedList *list, const char *element) {
    for (Node *curr = list->head; curr != NULL; curr = getNext(curr)) {
        if (strcmp(element, getData(curr)) == 0) { return true; }
    }
    return false;
}
...
int lastIndexOf(const LinkedList *list, const char *element) {
    ...
    int i = 0;
    for (Node *curr = list->head; curr != NULL; curr = getNext(curr)) {
        if (strcmp(getData(curr), element) == 0) { lastIndex = i; }
        i++;
    }
    return lastIndex;
}
...
const char *removeFirst(LinkedList *list) {
    ...
    const char *headData = getData(list->head);
...
const char *removeLast(LinkedList *list) {
    if (list->head == NULL) {
        ...
        const char *last = getData(list->head);
    ...
    const char *last = getData(getNext(end));
    removeNext(end);
    return last;
}
/*
...
bool removeFirstOccur(LinkedList *list, const char *element) {
    if (list->head == NULL) {
        return false;
    } else if (strcmp(getData(list->head), element) == 0) {
        removeFirst(list);
        return true;
    }

    Node *beforeNode = list->head;
    while (getNext(beforeNode) != NULL && strcmp(getData(getNext(beforeNode)), element) != 0) {
...
const char *removeAt(LinkedList *list, int index) {
    ...
    const char *element = getData(getNext(beforeIndex));
...
const char *removeLastOccur(LinkedList *list, const char *element) {
    ...
    for (Node *curr = list->head; getNext(curr) != NULL; curr = getNext(curr)) {
        if (strcmp(getData(getNext(curr)), element) == 0) {
            beforeLastOccur = curr;
        }
    }

    // if last occurrence of the element is at head only
    if (beforeLastOccur == NULL && strcmp(getData(list->head), element) == 0) {
        return removeFirst(list);
...
char **toArray(const LinkedList *list) {
    ...
    for (Node *curr = list->head; curr != NULL; curr = getNext(curr)) {
        arr[i] = malloc(sizeof(char) * (strlen(getData(curr)) + 1));
        strcpy(arr[i], getData(curr));
        i++;
    }
