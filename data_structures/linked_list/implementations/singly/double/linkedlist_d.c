...
#include "../node/double/node_singly_d.h"
 ...
 */
Node *insertAfter(Node *node, double data);
...
double removeFirst(LinkedList *list) {
    ...
    double headData = getData(list->head);
...
double removeLast(LinkedList *list) {
    ...
    double last = getData(getNext(end));
    removeNext(end);
    return last;
}
/*
...
double removeAt(LinkedList *list, int index) {
    ...
    double element = getData(getNext(beforeIndex));
