...
DynamicArray *newDynamicArray() {
    ...
    dynamicArray->array = calloc(DEFAULT_SIZE, sizeof(double));
    dynamicArray->size = 0;
 ...
 */
void addAt(DynamicArray *arr, int index, double element) {
 ...
 */
void addLast(DynamicArray *arr, double element) {
    ensureCapacity(arr, arr->size + 1);
 ...
 * of the array
 */
void addAll(DynamicArray *arr, const double *array, int sizeOfArray) {
    if (array == NULL)
        return;
...
void clear(DynamicArray *arr) {
    memset(arr->array, 0, sizeof(double) * arr->size);
}
 ...
 */
double getAt(const DynamicArray *arr, int index) {
 ...
 */
int lastIndexOf(const DynamicArray *arr, double val) {
    int lastIndex = -1;
...
*/
int indexOf(const DynamicArray *arr, double val) {
    for (int i = 0; i < arr->size; i++)
        if (arr->array[i] == val)
            return i;
 ...
 */
double removeAt(DynamicArray *arr, int index) {
    if (!isValidIndex(arr, index)) {
        return -1;
    }

    double element = arr->array[index];
 ...
 */
bool removeFirstOccur(DynamicArray *arr, double val) {
    for (int i = 0; i < arr->size; i++) {
        if (arr->array[i] == val) {
            removeAt(arr, i);
            return true;
 ...
 */
double setAt(DynamicArray *arr, int index, double val) {
    if (isValidIndex(arr, index)) {
        arr->array[index] = val;
        return val;
    }
...
void ensureCapacity(DynamicArray *arr, int minCapacity) {
    ...
    arr->array = realloc(arr->array, sizeof(double) * arr->capacity);
}
